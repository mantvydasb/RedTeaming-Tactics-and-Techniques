...
Sometimes it is possible to escalate privileges by abusing misconfigured services. Specifically, this is possible if path to the service binary is not wrapped in quotes and there are spaces in the path.. This stems from how Windows handles `CreateProcess` API calls:
...
The above suggests that if we can drop our binary to `c:\program.exe`, we may be able to stop/start the vulnerable service and get our binary at c:\program.exe to run with NT\System privileges:

![](../../.gitbook/assets/annotation-2019-05-20-222415.png)

## Execution

Let's try exploiting the weakness in the way the service is configured by droping a meterpreter binary to c:\program.exe and starting the vulnerable service `VulnerableSvc` - this should give us a meterpreter session with `nt authority\system` privileges:
